...
export function buildOperationLineage(history: readonly HistoryEvent[]): ReadonlyMap<string, LineageNode> {
  ...
  for (const event of history) {
    ...
    if (event.type === "invoke") {
      if (!mutable.has(operation.id)) mutable.set(operation.id, { operation, parent: operation.parentId, children: [], invokedAt: event.at });
      ...
      if (node) { node.completedAt = event.at; node.completion = event.type; }
    }
  }
  ...
  return new Map([...mutable.entries()].map(([id, node]) => [id, { ...node, children: [...node.children] }]));
}

export function operationAncestors(lineage: ReadonlyMap<string, LineageNode>, operationId: string): readonly OperationRef[] {
...
export function operationDescendants(lineage: ReadonlyMap<string, LineageNode>, operationId: string): readonly OperationRef[] {
  const result: OperationRef[] = [];
...
export class LineageFrontier {
  ...
  async expand(history: readonly HistoryEvent[], resolve: FaultSpaceResolver): Promise<LineageFrontierExpansion> {
    ...
    for (const call of newCalls) {
      ...
      this.#callCount++;
      const choices = (await resolve(call)).filter((choice) => !this.#seenPerturbations.has(choice.id)).map((choice) => ({
        ...choice,
        selector: choice.selector ?? { target: call.target, operation: call.operation, resource: call.resource, callsite: call.callsite, executionIndex: call.executionIndex },
      }));
      if (!choices.length) continue;
      for (const choice of choices) this.#seenPerturbations.add(choice.id);
      points.push({ id: `lineage:${call.key}`, target: call.target, choices });
    }
    return { newCalls, newFaultPoints: points, totalCalls: this.discoveredCalls, totalPerturbations: this.discoveredPerturbations };
  }
}

export interface LineageDiscoveryOptions { seed?: number; maxRounds?: number; maxRuns?: number; }
...
export async function discoverLineageFaultSpace<State>(
  ...
  let pending: FaultPoint[] = [...(await frontier.expand(baseline.history, resolve)).newFaultPoints];
  let rounds = 0;
  ...
  while (pending.length && rounds < maxRounds && runs.length < maxRuns) {
    ...
    for (const point of current) {
      for (const choice of point.choices) {
        ...
        const run = await execute({ id: `lineage-discovery:${choice.id}`, perturbations: [choice], seed: options.seed, metadata: { strategy: "lineage-discovery", round: rounds } });
        runs.push(run);
  ...
  return { baseline, runs, faultPoints: allPoints, rounds, discoveredCalls: frontier.discoveredCalls };
}
